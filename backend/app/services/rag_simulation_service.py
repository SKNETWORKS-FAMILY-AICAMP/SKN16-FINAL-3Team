...
"""
            ...
            # 대화 유형 판별
            conversation_type = "일반_상담_절차규제"  # 기본값
            is_product_manual = False
            has_product = False
            intent_type = ""
            category = ""
            
            # starter_topics에서 첫 번째 항목의 정보 확인
            if starter_topics and len(starter_topics) > 0:
                first_topic = starter_topics[0]
                is_product_manual = first_topic.get('is_from_product_manual', False)
                has_product = first_topic.get('product') is not None
                intent_type = first_topic.get('intent', '')
                
                # 카테고리 판별 (situation_id 기반)
                if situation_id == 'deposit':
                    category = "수신"
                elif situation_id == 'loan':
                    category = "여신"
                elif situation_id == 'card':
                    category = "카드"
                elif situation_id == 'fx':
                    category = "외환/송금"
                
                # 대화 유형 판별
                if is_product_manual and has_product:
                    conversation_type = "상품_설명서_기반"
                elif not is_product_manual and has_product:
                    conversation_type = "일반_상담_상품관련"
                elif not is_product_manual and not has_product:
                    if intent_type in ['기타문의', '세금수수료', '이용방법']:
                        conversation_type = "일반_상담_절차규제"
                    elif intent_type in ['환전문의', '송금문의'] or situation_id == 'fx':
                        conversation_type = "외환송금"
                    else:
                        conversation_type = "일반_상담_상품관련"
            
            # 대화 유형별 지식 평가 기준 구성
            knowledge_evaluation_guide = ""
            if conversation_type == "상품_설명서_기반":
                knowledge_evaluation_guide = """
**지식 평가 점수 구성 (100점):**
- 상품 정보 정확성: 70점 (제품 지식 자동 검증 결과 반영)
  * 금리, 한도, 조건 등 핵심 정보의 정확성
  * 우대조건, 수수료 등 부가 정보의 정확성
  * 정보 중요도별 가중치:
    - 핵심 정보 (금리, 한도, 조건): 오류 시 -15점
...
- 일반 금융 지식: 10점
  * 금융 상식, 상품 비교 기준 등
- 카테고리별 특화 지식: 5점
  * 수신: 예금 상식, 이자 계산, 예금자보호 한도, 중도해지 손실, 단리/복리 차이 등
  * 여신: 대출 상식, 신용점수 영향, 상환방식, 연체 이자, 중도상환 수수료 등
  * 카드: 카드 상식, 결제 시스템, 리볼빙, 최소결제금, 연회비 등
  * 외환/송금: 환율 기준 시점, 송금 수수료, 스위프트 코드 등
"""
            elif conversation_type == "일반_상담_상품관련":
                knowledge_evaluation_guide = """
**지식 평가 점수 구성 (100점):**
- 일반 상품 지식 정확성: 40점
...
- 일반 금융 지식: 20점
  ...
  * 금융 상식의 정확성 평가
- 카테고리별 특화 지식: 10점
  * 수신: 적금 설계, 목표 금액 역산, 예금자보호 한도 등
  * 여신: 대출 관리, 상환 순서, 연체 이자 등
  * 카드: 카드 사용, 혜택 활용, 리볼빙 등
  * 외환/송금: 환율 정보, 송금 절차 등
"""
            elif conversation_type == "일반_상담_절차규제":
                knowledge_evaluation_guide = """
**지식 평가 점수 구성 (100점):**
- 은행 업무 절차 지식 정확성: 50점
...
- 카테고리: {category}
- Intent: {intent_type}
- 상품 설명서 기반: {is_product_manual}
- 상품 정보 존재: {has_product}
...
**2️⃣ 기술 (Skill, 0-100점)**
- 목적: 응대 절차가 체계적이며 목표를 달성했는가 (⚠️ 페르소나와 무관한 절차적 역량 평가)
...
- 대화 흐름: 20점
  ...
  * 정보 제공 및 상담 진행 (7점)
  * 적절한 마무리 (3점)
- 목표 달성도: 60점 (상황별 Goals 반영)
  * 목표 달성률: {len(achieved_goal_indices)}/{len(goals) if goals else 0}개 달성 ({goal_achievement_rate*100:.0f}%)
  * 각 목표별 구체적 요구사항 달성 여부 평가
...
**단순 업무 처리 대화 평가 기준 (절차/규제 문의인 경우):**
- 업무 절차 준수: 35점
  * 필수 서류 확인 여부: 12점
  * 절차 단계 순서 준수: 12점
  * 필수 확인 사항 체크: 11점
...
- 추가 확인 및 안내: 10점
  * 추가 서류 필요 여부 안내: 5점
  * 다음 단계 안내: 5점
""" if conversation_type == "일반_상담_절차규제" else ""}
...
- 평가 기준:
  ...
  ⚠️ **고객 성격 유형별 대응은 페르소나 정합도에서 평가하므로, 기술에서는 평가하지 않음**
- 피드백 작성 시: 
...
- 목적: 고객 중심의 배려 있는 언어를 사용했는가 (⚠️ 페르소나와 무관한 일반적인 친절도 평가)
...
- 평가 기준:
  ...
  ✓ 고객의 불편/불만에 대한 일반적인 공감 및 사과: "불편을 드려 죄송합니다", "기다려주셔서 감사합니다"
    ⚠️ **참고**: 불만형 고객의 불만 상황별 구체적인 대응 전략(시점, 품질, 순서)은 페르소나 정합도에서 평가
  ...
  ✓ 고객의 시간 존중: "시간 내주셔서 감사합니다"
    ⚠️ **참고**: 급함형 고객의 급함 상황별 구체적인 대응 전략(빠른 처리 의지, 간결성 등)은 페르소나 정합도에서 평가
  ...
  ⚠️ **고객 성격 유형별 특화된 대응 전략(불만형/급함형/긍정형별 세부 평가)은 페르소나 정합도에서 평가하므로, 친절도에서는 평가하지 않음**

- 피드백 작성 시: 
  ✓ 친절했던 표현을 구체적으로 인용하여 칭찬
  ✓ 개선이 필요한 표현은 Before → After 형식으로 제시
  ✓ 고객의 불편/불만에 대한 일반적인 대응 여부 평가 (페르소나별 특화 전략은 페르소나 정합도에서 평가)
...
**⚠️ 중요: promptOrchestrator.py의 불만형 고객 대화 지침을 참고하여 평가하세요**
- 불만형 고객은 일반적인 대화에서는 자연스럽게 답함 (매번 불만 표현 아님)
- 불만은 **적절한 상황**에서만 표현: 직원 실수, 처리 지연(3턴 이상), 문제 발생, 설명 불충분, 대화 반복 등
- 첫 1-2턴에서는 불만 표현 없음 (상담 시작 전이므로)
- 직원의 정당한 질문/설명에는 자연스럽게 답함
- 직원이 해결책 제시 → 고객이 수용 ("그럼 그렇게 해주세요")

1. **공감 및 사과 표현 (50점) - 문맥 중심 평가**
   
   **A-1. 불만 상황 파악 및 적절한 시점 대응 (25점)**
   ✓ 불만 표현이 적절한 상황에서 공감/사과했는지 평가:
     - **불만 표현이 적절한 상황 파악**:
       * 직원 실수 발생 시 → 즉시 공감/사과 (1턴 이내): 25점
       * 처리 시간 지연 (3턴 이상) → 즉시 공감/사과 (1턴 이내): 25점
       * 문제 발생 시 → 즉시 공감/사과 (1턴 이내): 25점
       * 설명 불충분/애매 → 즉시 공감/사과 (1턴 이내): 25점
       * 대화 반복/길어짐 → 즉시 공감/사과 (1턴 이내): 25점
     - 불만 상황 파악 후 약간 늦게 (2-3턴) 공감/사과: 15점
     - 불만 상황 파악 후 너무 늦게 (4턴 이상) 공감/사과: 8점
     - **불만 상황이 아닌데 공감/사과** (부적절한 시점): 3점
       * 예: 고객이 "네, 정기예금이요"라고 정상 답변했는데 공감/사과
       * 예: 고객이 일반 질문만 했는데 공감/사과
     - 불만 상황인데 공감/사과 없음: 0점
   ...
   **A-3. 대화 흐름의 자연스러움 (10점)**
   ✓ 공감/사과가 대화 흐름상 자연스러운가:
     - 불만 상황 발생 → 즉시 공감/사과 → 해결책 제시 순서: 10점
     - 불만 상황 → 공감/사과 (순서는 맞으나 약간 어색): 7점
     - 불만 상황 → 해결책만 → 나중에 공감/사과: 4점 (순서 문제)
     - 불만 상황과 무관한 시점에 공감/사과: 2점
     - 공감/사과가 전혀 없음: 0점
   
   ✗ 감점 요인:
     - 불만을 무시하거나 방어적인 태도 (예: "그건 우리 책임이 아닙니다"): -15점
     - 책임 회피 표현 (예: "저는 그런 지시를 받지 못했습니다"): -10점
     - 고객 불만 상황 직후에도 공감/사과 없이 본론만 진행: -10점
     - 최소 0점
...
2. **해결책 제시 (40점) - 문맥 중심 평가**
   ...
   **B-1. 해결책 제시의 타이밍 및 순서 (20점)**
   ✓ 공감 후 해결책 제시 여부 평가:
     - 불만 상황 → 공감/사과 → 즉시 해결책 제시: 20점 (완벽한 순서)
     - 불만 상황 → 공감/사과 → 약간 늦게 해결책 제시: 15점
     - 불만 상황 → 해결책만 제시 (공감 없음): 10점 (순서 문제)
     - 불만 상황 후 해결책 제시 없음: 0점
...
**점수 계산 예시 (불만형):**
**상황 1**: 고객이 "왜 이렇게 오래 걸려요?" 불만 표현 (처리 지연 상황)
- 불만 상황 파악 후 즉시 공감/사과 (1턴 이내): 25점
- 구체적이고 진정성 있는 공감 ("불편을 드려 정말 죄송합니다. 기다려주셔서 감사합니다"): 15점
- 자연스러운 대화 흐름 (불만 상황 → 공감 → 해결책): 10점
- 공감 후 즉시 해결책 제시: 20점
- 구체적이고 실현 가능한 해결책 ("바로 처리해 드리겠습니다"): 15점
- 고객 불만과 직접 관련된 해결책: 5점
- 부정 패턴 없음: 10점
- **총점: 100점**

**상황 2**: 고객이 일반 질문만 함 ("정기예금에 대해 알고 싶어요") - 불만 상황 아님
- 불만 상황이 아닌데 공감/사과 (부적절한 시점): 3점
- 공감 표현 없음 (정상): 0점
- 자연스러운 대화 흐름 유지: 10점
- **총점: 13점** (불만 상황이 아니므로 공감/사과는 필요 없음)

**B. 급함형 고객 (타입이 "급함형"인 경우)**
점수 구성: 문맥적_빠른대응 (40점) + 문맥적_간결성 (40점) + 문맥적_핵심전달 (20점) = 100점

**⚠️ 중요: promptOrchestrator.py의 급함형 고객 대화 지침을 참고하여 평가하세요**
- 급함형 = 간결한 응답 + 빠른 결정 (표현 반복이 아님!)
- 급함 표현은 **최대 1-2회만** 사용 (처음 방문 목적 말할 때 1회, 실제 지연 발생 시 1회)
- 매 턴마다 "빨리" 반복하지 않음
- 직원이 처리 중이라고 하면 → "네, 알겠습니다" 같은 짧은 응답만 (급함 표현 재사용 안 함)

1. **빠른 처리 표현 및 대응 (40점) - 문맥 중심 평가**
   
   **A-1. 고객 급함 상황 파악 및 즉각 대응 (20점)**
   ✓ 고객이 급함을 표현한 직후 빠른 처리 의지 표명 여부:
     - 고객이 급함을 표현한 직후 (1턴 이내) 빠른 처리 의지 명확히 표시 (예: "바로 처리해 드리겠습니다"): 20점
       * 급함 표현 예: "빨리 처리할 수 있을까요?", "빨리 진행하고 싶어요", "지금 바로 가능할까요?"
     - 고객 급함 표현 직후 빠른 처리 의지 있으나 약간 모호: 15점
     - 고객 급함 표현 후 약간 늦게 빠른 처리 의지 표시: 10점
     - 고객 급함 표현이 없는데도 빠른 처리 표현 사용: 5점 (부적절한 시점)
       * ⚠️ **참고**: 급함형 고객이어도 일반 대화에서는 급함 표현 없이 자연스럽게 대화함
   ...
   **A-3. 대화 흐름상 빠른 대응의 자연스러움 (5점)**
   ...
   ✗ 감점 요인 (문맥적으로 평가):
     - 고객의 급함을 인지하지 못하고 처리 시간을 지연시킬 뿐인 경우: -10점
       * 예: 고객이 "빨리 해야 하는데요"라고 했는데 "시간이 좀 걸릴 수 있습니다"만 말하고 빠른 처리 의지 없음
     - 급함 상황을 무시하거나 경시하는 태도: -15점
       * 예: "급하게 하시면 문제가 생길 수 있습니다"처럼 고객의 급함을 부정하는 표현
     - ⚠️ **참고**: 처리 시간이 정말 필요한 상황에서 "잠시만 기다려주세요", "시간이 걸릴 수 있습니다"라고 양해를 구하면서 **동시에 빠른 처리 의지를 표시**한다면 감점하지 않음
       * 예: "잠시만 기다려주세요. 바로 처리해 드리겠습니다" (양해 구하기 + 빠른 처리 의지) → 감점 없음
     - 최소 0점
...
**점수 계산 예시 (급함형):**
**상황 1**: 고객이 처음 방문 목적 말할 때 "빨리 진행하고 싶어요" 급함 표현
- 고객 급함 표현 직후 1턴 내 "바로 처리해 드리겠습니다": 20점
...
**상황 2**: 고객이 일반 대화만 함 ("네, 체크카드로 해주세요") - 급함 표현 없음
- 급함 표현 없이 자연스러운 대화 (정상): 10점
- 빠른 처리 의지 없음 (급함 표현 없으므로 필요 없음): 10점
- 일반 대화 흐름 유지: 5점
- 급함 상황 고려한 간결한 설명: 20점
- 핵심 정보 전달: 10점
- 연결어 최소 사용: 5점
- 질문에 직접 답변: 12점
- 핵심 정보 먼저: 8점
- **총점: 80점** (급함 표현이 없어도 급함형 고객에게는 간결하고 효율적으로 대응하면 점수 획득)

**C. 긍정형 고객 (타입이 "긍정형"인 경우)**
점수 구성: 기본_친절도유지 (35점) + 협조적대응 (30점) + 적절한추가안내 (35점) = 100점

**⚠️ 중요: promptOrchestrator.py의 긍정형 고객 대화 지침을 참고하여 평가하세요**
- 긍정형 고객은 밝은 톤 ("좋네요!", "감사합니다!", "그럼 바로 할게요.")
- 하지만 매번 긍정적으로 반응할 필요 없음 (자연스럽게 대화)
- 가벼운 긍정 리액션(예: "네", "좋아요", "감사합니다")은 평범한 응답
- 진심 어린 긍정 표현(예: "정말 도움이 되었어요", "완벽해요!")에만 적절히 긍정적으로 대응
- 전반적으로 친절하고 긍정적인 분위기 유지 + 효율적 업무 진행이 중요
- 협조적이므로 불필요하게 설명 늘리거나 우회하지 말고, 정확하고 친절하게 정보 제공

1. **기본 친절도 유지 (35점) - 문맥 중심 평가**
   
   **A-1. 전반적인 친절한 톤 유지 (20점)**
   ✓ 대화 전반에 걸쳐 친절하고 긍정적인 톤 유지:
     - 전반적으로 일관되게 친절하고 긍정적인 톤 유지 (예: "감사합니다", "도와드리겠습니다", "편리하시면", "괜찮으시면"): 20점
     - 대부분 친절한 톤 유지, 일부 형식적: 15점
     - 친절한 톤과 중립적 톤 혼재: 10점
     - 중립적이거나 다소 딱딱한 톤: 5점
     - 부정적이거나 무뚝뚝한 톤: 0점
   
   **A-2. 부정적 톤 및 표현 회피 (15점)**
   ✓ 긍정형 고객에게 부적절한 표현을 사용하지 않았는가:
     - 부정적 표현 전혀 없음: 15점
     - 부정적 표현 1회 사용 (예: "안 됩니다", "불가능합니다"): 8점
     - 부정적 표현 2회 이상: 3점
     - 고객의 협조적 태도에도 부정적 톤 사용: 0점
     - ⚠️ **참고**: 부정적 표현은 부정적 톤 사용을 의미하며, 정직하게 "불가능합니다"라고 해야 하는 상황이라면 감점하지 않음 (문맥 고려)
   
   ✗ 감점 요인:
     - 긍정형 고객에게 명령형 톤 사용 (예: "하세요", "하시죠"): -10점
     - 고객의 협조적 태도를 당연시하거나 무시하는 태도: -15점
     - 최소 0점

2. **협조적 태도에 맞춘 효율적 대응 (30점) - 문맥 중심 평가**
   
   **B-1. 고객의 빠른 결정 성향에 맞춘 효율적 대응 (15점)**
   ✓ 긍정형 고객은 보통 빠르게 결정하므로, 효율적으로 진행했는가:
     - 고객이 빠르게 결정할 때 효율적으로 진행하고 불필요한 설명 회피: 15점
     - 대부분 효율적으로 진행, 약간의 불필요한 설명: 12점
     - 효율성과 설명의 균형 유지: 8점
     - 불필요하게 설명을 늘리거나 우회: 4점
     - 긍정형 고객에게도 장황하고 비효율적인 진행: 0점
   
   **B-2. 고객의 진심 어린 긍정 표현에 대한 적절한 대응 (10점)**
   ✓ 고객이 진심 어린 긍정 표현을 할 때 적절히 대응했는가:
     - 고객의 진심 어린 긍정 표현(예: "정말 도움이 되었어요", "완벽해요!")에 자연스럽게 긍정적으로 대응: 10점
     - 진심 어린 긍정 표현에 적절히 대응하나 약간 어색: 7점
     - 진심 어린 긍정 표현에 형식적으로만 대응: 4점
     - 진심 어린 긍정 표현을 무시하거나 부정적으로 대응: 0점
     - ⚠️ **중요**: 가벼운 긍정 리액션(예: "네", "좋아요", "감사합니다")은 평가하지 않음. 진심 어린 긍정 표현만 평가
   
   **B-3. 고객의 협조적 태도 활용 (5점)**
   ✓ 긍정형 고객의 협조적 태도를 적절히 활용하여 효율적으로 진행했는가:
     - 고객의 협조적 태도에 맞춰 정확하고 친절하게 정보 제공: 5점
     - 고객의 협조적 태도를 당연시하거나 무시: 2점
     - 고객의 협조적 태도를 오히려 부담스러워하는 태도: 0점
   
   ✗ 감점 요인:
     - 긍정형 고객의 가벼운 긍정 리액션에도 과도하게 긍정적으로 반응 (부자연스러움): -5점

3. **적절한 추가 안내 제공 (35점) - 문맥 중심 평가**
   
   **C-1. 고객의 관심 및 필요 파악 후 추가 안내 (20점)**
   ✓ 고객이 추가 정보에 관심이 있어 보이거나 필요할 때 제공했는가:
     - 고객이 추가 정보를 직접 묻거나 관심을 보일 때 적절히 제공 (예: 고객 "그 외에는?", "다른 상품은?", "관련 서비스는?"): 20점
     - 고객이 묻지 않았지만 맥락상 추가 정보가 필요해 보일 때 제공: 15점
     - 고객이 추가 정보에 관심을 보였는데 제공하지 않음: 8점
     - 고객의 관심과 무관하게 무작정 추가 안내 제공: 5점
     - 추가 안내 전혀 제공하지 않음: 0점
     - ⚠️ **중요**: 긍정형 고객이라고 해서 무조건 추가 안내를 해야 하는 것이 아님. 고객의 관심이나 필요가 있을 때만 제공
   
   **C-2. 추가 안내의 품질 및 고객 상황 적합성 (12점)**
   ✓ 제공한 추가 안내가 고객 상황에 맞는가:
     - 고객의 현재 상황이나 관심사에 맞는 매우 유용한 추가 안내: 12점
     - 고객에게 유용한 추가 안내: 9점
     - 일반적인 추가 안내: 6점
     - 고객 상황과 맞지 않는 추가 안내: 3점
     - 추가 안내 없음: 0점
   
   **C-3. 친절한 마무리 및 지속적 관심 표현 (3점)**
   ✓ 대화 마무리 시 친절하고 지속적인 관심을 표현했는가:
     - 친절하고 지속적인 관심 표현 (예: "추가로 궁금한 점 있으시면 언제든지 문의해 주세요"): 3점
     - 기본적인 친절한 마무리 (예: "감사합니다"): 2점
     - 형식적인 마무리: 1점
     - 마무리 없음: 0점

**점수 계산 예시 (긍정형):**
- 전반적으로 일관되게 친절하고 긍정적인 톤 유지: 20점
- 부정적 표현 전혀 없음: 15점
- 긍정형 고객이 빠르게 결정할 때 효율적으로 진행: 15점
- 고객 "정말 도움이 되었어요" 진심 어린 긍정 표현에 자연스럽게 대응: 10점
- 고객의 협조적 태도에 맞춰 정확하고 친절하게 정보 제공: 5점
- 고객이 "그 외에는?" 관심 표현할 때 적절히 추가 안내 제공: 20점
- 고객 상황에 맞는 매우 유용한 추가 안내: 12점
- 친절하고 지속적인 관심 표현: 3점
- **총점: 100점**

**⚠️ 평가 시 주의사항:**
1. 고객의 가벼운 긍정 리액션(예: "네", "좋아요", "감사합니다")은 평범한 응답이므로, 매번 과도하게 긍정적으로 반응할 필요 없음. 이는 평가하지 않음.
2. 진심 어린 긍정 표현(예: "정말 도움이 되었어요", "완벽해요!", "잘 알아듣겠어요")에만 적절히 긍정적으로 대응했는지 평가.
3. 전반적인 친절도와 긍정적 분위기 유지가 가장 중요함. 개별적인 긍정 표현보다는 전체적인 톤과 분위기를 평가.
4. 긍정형 고객은 협조적이므로, 효율적이고 정확한 대응이 중요함. 불필요하게 설명을 늘리거나 우회하는 것은 감점.
...
0. **대화 유형 판별 (먼저 수행):**
   - 위에서 확인한 대화 유형 정보를 바탕으로 적절한 평가 기준을 적용하세요
   - 상품 설명서 기반 상담: 상품 정보 정확성(70점) + 절차/규제(15점) + 일반 지식(10점) + 특화 지식(5점)
   - 일반 상담 - 상품 관련: 일반 상품 지식(40점) + 절차/규제(30점) + 일반 지식(20점) + 특화 지식(10점)
   - 일반 상담 - 절차/규제: 절차 지식(50점) + 규정/정책(30점) + 창구 업무(20점)
   - 외환/송금: 절차 설명(40점) + 환율/수수료(40점) + 외환 규제(20점)

1. **⚠️⚠️⚠️ 반드시 promptOrchestrator.py의 페르소나별 대화 지침을 참고하여 평가하세요**
   - **불만형**: 일반 대화에서는 자연스럽게 답함 (매번 불만 아님). 불만은 적절한 상황(직원 실수, 지연 3턴 이상, 문제 발생 등)에서만 표현. 첫 1-2턴에서는 불만 표현 없음
   - **급함형**: 급함 표현은 최대 1-2회만 (처음 1회, 실제 지연 발생 시 1회). 일반 대화에서는 급함 표현 없이 자연스럽게 대화. 급함형 = 간결한 응답 + 빠른 결정 (표현 반복이 아님!)
   - **긍정형**: 밝은 톤 유지하되 매번 긍정적으로 반응할 필요 없음. 자연스럽게 대화하면서 전반적으로 친절한 분위기 유지

2. **반드시 위에서 확인한 고객 타입에 맞는 평가 기준을 적용하세요**

3. **빈도보다 문맥(적절한 시점, 대화 흐름, 품질)을 중시하여 평가하세요**
   - 단순히 키워드나 표현의 개수를 세는 것이 아닌, **언제, 어떤 상황에서, 어떻게** 사용했는지를 평가하세요
   - 예: 불만형 고객이 일반 질문만 했는데 공감/사과를 했다면 → 부적절한 시점 (낮은 점수)
   - 예: 급함형 고객이 급함 표현 없이 일반 대화만 했는데 급함 표현을 사용했다면 → 부적절한 시점 (낮은 점수)
   - 예: 공감 표현이 1회만 있어도 고객 불만 상황 직후 적절한 시점에 구체적으로 사용했다면 높은 점수를 부여하세요

4. **대화 흐름과 맥락을 정확히 파악하세요**
   - 고객의 발화를 먼저 읽고, **그것이 불만 상황인지, 급함 표현인지, 일반 대화인지** 정확히 판단하세요
   - 불만 상황이 아닌 일반 대화에서 공감/사과를 했다면 부적절한 시점입니다
   - 급함 표현이 없는 일반 대화에서 급함 표현을 사용했다면 부적절한 시점입니다
...
7. **중복 제거**: 
   - 기술(Skill): 상담 절차와 목표 달성도만 평가 (페르소나 무관)
   - 친절도(Kindness): 일반적인 친절한 태도와 배려만 평가 (페르소나 무관)
   - 페르소나 정합도(Persona Fit): 페르소나별 특화된 대응 전략 전부 평가 (불만형/급함형/긍정형별 세부 평가)
   - 각 역량은 서로 겹치지 않도록 명확히 구분하여 평가하세요
...
**⚠️ 중복 제거 가이드:**
- **역량별 역할 분담** (중복 없이 명확히 구분):
  - 지식(Knowledge): 상품 정보의 정확성만 평가
  - 기술(Skill): 상담 절차와 목표 달성도만 평가 (페르소나 무관)
  - 친절도(Kindness): 일반적인 친절한 태도와 배려만 평가 (페르소나 무관)
  - 전달력(Clarity + Confidence): 정보 전달의 명확성과 자신감만 평가
  - 페르소나 정합도(Persona Fit): 페르소나별 특화된 대응 전략 전부 평가 (불만형/급함형/긍정형별 세부 평가)
- **역량 간 중복 피하기**:
  - 지식 평가에서 이미 상세히 다룬 오류(예: "최소 100")는 전달력에서 간단히 참조만
  - 예: "지식 평가에서 언급한 '최소 100' 표현은 명확성 측면에서도 개선이 필요합니다"
  - 페르소나별 대응 전략은 페르소나 정합도에서만 평가하고, 기술이나 친절도에서는 평가하지 않음
  - 같은 내용을 여러 역량에서 반복 설명하지 말고, 각 역량의 관점에서만 평가
...
{{
    "knowledge": {{
        "score": <0-100 점수, 위에서 확인한 대화 유형에 맞는 점수 구성 적용>,
        "feedback": "<마크다운 형식, **잘한 점** 섹션은 필수, **개선점** 섹션은 개선할 점이 있을 때만 작성. **상품 정보의 정확성**에만 집중하여 피드백 작성. 🚨 **중요: 위 제품 지식 자동 검증 결과의 '정확한 정보 목록'에 있는 claim만 잘한 점에 언급하고, '부정확한 정보 목록'에 있는 claim만 개선점에 언급하세요. 같은 claim이 잘한 점과 개선점에 동시에 나타나면 안 됩니다 (모순 금지).** 구체적 예시는 **볼드**로 강조. 부정확한 정보는 정확한 정보와 함께 제시 (예: **'금리 3.5%'** → **'실제로는 2.15%'**). 제품 지식 자동 검증 결과의 LLM reasoning 활용. ⚠️ 표현의 명확성(단위 명시, 용어 평이성)은 전달력에서 다루므로 지식 피드백에서 언급하지 않음. ⚠️ 점수가 100점이면 모든 정보가 정확하다는 의미이므로 개선점 섹션은 생략하거나 '제공한 모든 상품 정보가 정확합니다'와 같이 간단히 언급. 대화 유형별 평가 범위: 상품 설명서 기반(상품 정보 70점 + 절차/규제 15점 + 일반 지식 10점 + 특화 지식 5점), 일반 상담-상품 관련(일반 상품 지식 40점 + 절차/규제 30점 + 일반 지식 20점 + 특화 지식 10점), 일반 상담-절차/규제(절차 지식 50점 + 규정/정책 30점 + 창구 업무 20점), 외환/송금(절차 설명 40점 + 환율/수수료 40점 + 외환 규제 20점)>"
    }},
    "skill": {{
        "score": <0-100 점수, 대화 흐름(20점) + 목표 달성도(60점) + 질문 사용(10점) + 피드백 루프(10점)>,
        "feedback": "<마크다운 형식, **잘한 점**과 **개선점** 섹션으로 구분. 대화 흐름(인사 3점, 요구사항 파악 7점, 정보 제공 7점, 마무리 3점)과 목표 달성도(상황별 Goals 반영, 60점) 평가, 구체적 개선 제안. 달성한 목표와 미달성한 목표를 명시하고, 미달성 목표에 대한 개선 방안 제시. 목표 텍스트에 명시된 구체적 키워드(인용부호 내 항목, 나열된 항목 등)가 실제로 다뤄졌는지 확인하여 피드백 작성>"
    }},
    "clarity": {{
        "score": <0-100 점수, 문장 구조(30점) + 논리성(25점) + 용어 평이성(30점) + 숫자 표현(15점)>,
        "feedback": "<마크다운 형식, **잘한 점**과 **개선점** 섹션으로 구분. 문장 구조와 용어 사용 평가, 쉬운 표현 제안. 모호한 표현은 Before → After 형식으로 제안 (예: **'최소 100'** → **'최소 100만원'**). 평균 문장 길이, 논리적 연결어 사용, 전문용어 사용 개수, 숫자 단위 명시 여부를 평가>"
    }},
    "kindness": {{
        "score": <0-100 점수, 기본 정중함(30점) + 선택권 존중(25점) + 공감(20점) + 추가 도움(10점) + 부정 표현 회피(15점)>,
        "feedback": "<마크다운 형식, **잘한 점**과 **개선점** 섹션으로 구분. 친절한 표현 사례와 개선 필요 표현 지적. Before → After 형식으로 제안 (예: **'더 빠르고 정확합니다'** → **'더 편리할 수 있습니다'**). 정중한 어투 유지, 고객 선택권 존중, 공감 표현, 추가 도움 의지, 부정 표현 회피 여부를 평가>"
    }},
    "confidence": {{
        "score": <0-100 점수, 확정적 표현 비율(80점) + 모호 표현 감점(20점)>,
        "feedback": "<마크다운 형식, **잘한 점**과 **개선점** 섹션으로 구분. 자신감 있는 어투와 불확실한 표현 비교. Before → After 형식으로 제안 (예: **'~같아요'** → **'~입니다'**). 지식 평가에서 언급한 부정확한 정보를 확신 있게 말한 경우도 언급. 확정적 표현 사용 비율(90% 이상: 80점, 80-89%: 60점 등)과 모호 표현 개수(0개: 20점, 1-2개: 10점 등)를 평가>"
    }},
    "clarity_confidence": {{
        "score": <명확성 60% + 자신감 40% (가중 평균), 0-100 점수>,
...
"""
            ...
            # 전달력 = 명확성 60% + 자신감 40% (가중 평균)
            # GPT가 clarity_confidence를 생성했으면 사용, 없으면 가중 평균 계산
            if 'clarity_confidence' in evaluation:
                ...
                # Fallback: 명확성과 자신감의 가중 평균
                clarity_score = evaluation.get('clarity', {}).get('score', 0)
                confidence_score = evaluation.get('confidence', {}).get('score', 0)
                clarity_confidence_score = round(clarity_score * 0.6 + confidence_score * 0.4)
    ...
    def _get_default_feedback(self) -> Dict:
        ...
        return {
            ...
            "competencies": [
                ...
                {"name": "페르소나 정합도", "score": 70, "maxScore": 100}
            ],
            "detailedFeedback": {
                ...
                "persona_fit": {
                    "score": 70,
                    "feedback": "고객의 페르소나 타입에 맞는 대응을 시도하고 있습니다."
                },
                # 하위 호환성을 위해 기존 필드도 유지 (deprecated)
