...
"""
            ...
            if 'has_product_data' in situation:
                ...
                # situation_id가 'fx'로 시작하거나 intent가 외환/송금 관련이면 상품 데이터 없음
                if (situation_id and situation_id.startswith('fx')) or intent in ['환전문의', '송금문의', '외환문의', '외환/송금']:
                    has_product_data = False
                    print(f"📋 외환/송금 상황 감지: situation_id={situation_id}, intent={intent} → has_product_data=False")
                else:
                    has_product_data = True  # 기본값: True
            ...
            if is_from_product_manual and product_code:
                ...
                # 일반 상담 (intent로 세부 구분)
                # situation_id가 'fx'로 시작하거나 intent가 외환/송금 관련이면 D 유형
                if (situation_id and situation_id.startswith('fx')) or intent in ['환전문의', '송금문의', '외환문의', '외환/송금']:
                    # D. 외환/송금 상담
                    conversation_type = "D"
                    conversation_type_description = "외환/송금 상담 (상품 데이터 없음, 절차 및 지식 중심)"
                    has_product_data = False  # 명시적으로 False 설정
                    print(f"📋 대화 유형: D (외환/송금) - Intent: {intent}, Situation ID: {situation_id}")
...
- 외환 규제 (20점): 제재 규제, 신고 의무 등
  ...
  * 신고 기준 안내"""
            ...
            if conversation_type == "A":
                knowledge_breakdown_structure = """            "product_accuracy": {{"score": <점수>, "max": 70, "reason": "<근거>"}},
            "procedure_knowledge": {{"score": <점수>, "max": 15, "reason": "<근거>"}},
            "general_finance": {{"score": <점수>, "max": 10, "reason": "<근거>"}},
            "category_specific": {{"score": <점수>, "max": 5, "reason": "<근거>"}}"""
            elif conversation_type == "B":
                knowledge_breakdown_structure = """            "product_knowledge": {{"score": <점수>, "max": 40, "reason": "<근거>"}},
            "procedure_knowledge": {{"score": <점수>, "max": 30, "reason": "<근거>"}},
            "general_finance": {{"score": <점수>, "max": 20, "reason": "<근거>"}},
            "category_specific": {{"score": <점수>, "max": 10, "reason": "<근거>"}}"""
            elif conversation_type == "C":
                knowledge_breakdown_structure = """            "procedure_knowledge": {{"score": <점수>, "max": 50, "reason": "<근거>"}},
            "regulation_policy": {{"score": <점수>, "max": 30, "reason": "<근거>"}},
            "general_banking": {{"score": <점수>, "max": 20, "reason": "<근거>"}}"""
            elif conversation_type == "D":
                knowledge_breakdown_structure = """            "procedure_explanation": {{"score": <점수>, "max": 40, "reason": "<근거>"}},
            "exchange_rate_fee_info": {{"score": <점수>, "max": 40, "reason": "<근거>"}},
            "foreign_exchange_regulation": {{"score": <점수>, "max": 20, "reason": "<근거>"}}"""
...
- 목적: **표현 방식이 명확하고 이해하기 쉬운가** (정보의 정확성이 아닌 전달 방식 평가)
...
- **논리성 (25점)**: 
  * 기승전결이 명확하고 인과관계가 맞음: 25점
  * 앞뒤 말이 안 맞거나 비약이 있음: 12점
...
2. **오직 표현 방식만 평가합니다** (용어 선택, 문장 구조, 숫자 단위 명시 등)
3. 예: "연회비는 무료부터 시작"이라는 표현 자체는 명확함 (사실 여부는 지식 평가)
...
- 목적: **어조의 확신성** (정보 정확성이나 표현 명확성이 아님)

**평가 기준:**
- **확정적 표현 (80점)**:
  * "가능합니다", "됩니다" 등 명확한 종결어미 사용 비율 80% 이상: 80점
  * "~같아요", "아마도" 등 불확실한 표현 남발: 40점
  
- **불확실성 회피 (20점)**: **(감점 방식)**
  * 모호한 표현("~같아요", "아마", "~인 것 같은데요") 없음: 20점
  * 모호한 표현 1-2회: 10점
  * 모호한 표현 3회 이상: 0점
...
1. **정보의 사실 관계(정확성)는 평가하지 않습니다** → 지식 평가 영역
2. **표현의 명확성은 평가하지 않습니다** → 명확성 평가 영역
3. **오직 어조의 확신성만 평가합니다** ("~입니다" vs "~같아요")
4. 예: "연회비는 무료부터 시작합니다" (확신 있음, OK) vs "무료인 것 같아요" (불확실, 감점)
...
🚨 **페르소나 평가의 핵심 원칙:**
1. **정보의 사실 관계(정확성)는 평가하지 않습니다** → 지식 평가 영역
2. **표현의 명확성/친절도/자신감은 평가하지 않습니다** → 각 역량 영역
...
{"- 제품 지식 자동 검증 결과와 실제 대화 내용을 대조하여 정확히 평가하세요" if conversation_type in ["A", "B"] else "- 절차 및 규제 지식의 정확성을 평가하세요"}
...
{knowledge_breakdown_structure}
    ...
    "clarity": {{
        ...
        "feedback": "<마크다운 형식, **잘한 점**과 **개선점** 섹션으로 구분. 문장 구조와 용어 사용 평가, 쉬운 표현 제안. 🚨 **필수: 반드시 실제 대화 로그에서 사용된 구체적인 표현을 찾아서 인용하세요.** 위 '대화 내용' 섹션을 꼼꼼히 검토하여 실제로 사용된 전문 용어나 모호한 표현을 찾아 인용하세요. Before → After 형식으로 제안하되, Before 부분은 반드시 실제 대화 로그에 있는 표현이어야 합니다. ❌ '일부 전문 용어' 같은 모호한 표현 금지, ✅ 실제 대화 로그에서 찾은 **'[구체적 용어]'** 인용 필수. ⚠️ **주의: 지식 평가에서 다룬 '정보의 정확성'(사실 관계) 문제는 여기서 다시 언급하지 마세요. 오직 표현 방식과 전달력 관점에서만 평가하세요.**>"
    }},
    "kindness": {{
        "score": <0-100 점수, breakdown의 모든 항목 점수 합산>,
        "breakdown": {{
        ...
        "feedback": "<마크다운 형식, **잘한 점**과 **개선점** 섹션으로 구분. 친절한 표현 사례와 개선 필요 표현 지적. 🚨 **필수: 반드시 실제 대화 로그에서 사용된 구체적인 표현을 찾아서 인용하세요.** 위 '대화 내용' 섹션을 꼼꼼히 검토하여 실제로 사용된 부정 표현을 찾아 인용하세요. Before → After 형식으로 제안하되, Before 부분은 반드시 실제 대화 로그에 있는 표현이어야 합니다. ❌ '부정 표현을 회피할 수 있도록' 같은 모호한 표현 금지, ✅ 실제 대화 로그에서 찾은 **'[구체적 부정 표현]'** 인용 필수. ⚠️ **주의: 지식 평가에서 다룬 '정보의 정확성'(사실 관계) 문제는 여기서 다시 언급하지 마세요. 오직 태도와 정중함 관점에서만 평가하세요.**>"
    }},
    "confidence": {{
        "score": <0-100 점수, breakdown의 모든 항목 점수 합산>,
        "breakdown": {{
...
"""
            ...
            print(f"📏 평가 프롬프트 길이: {prompt_length}자 (대화 유형: {conversation_type})")
            if prompt_length > 100000:  # 약 25,000 토큰 (한글 기준)
                print(f"⚠️ 프롬프트가 너무 깁니다 ({prompt_length}자). 일부 섹션을 축약합니다.")
            elif prompt_length > 50000:  # 약 12,500 토큰
                print(f"⚠️ 프롬프트가 깁니다 ({prompt_length}자). LLM 거부 가능성 있음.")
            ...
            # 🚨 LLM이 거부 응답을 반환한 경우 확인
            if not content or content.strip().startswith("I'm sorry") or "can't assist" in content.lower():
                print(f"❌ LLM이 요청을 거부했습니다. 응답: {content[:200]}")
                print(f"⚠️ 기본 피드백 반환 (LLM 거부)")
                raise ValueError(f"LLM이 평가 요청을 거부했습니다: {content[:200]}")
            ...
            try:
                ...
                try:
                    ...
                    print(f"📝 전체 응답 내용 (마지막 2000자): {content[-2000:]}")
                    raise e
            ...
            if not persona_fit_data:
                print(f"⚠️ evaluation에 persona_fit이 없습니다. evaluation 키: {list(evaluation.keys())}")
                print(f"📝 evaluation 내용 샘플: {str(evaluation)[:500]}")
                persona_fit_score = 50
                persona_fit_feedback = '페르소나 정합도 평가가 생성되지 않았습니다. LLM 응답에 persona_fit 필드가 없습니다.'
            else:
                persona_fit_score = persona_fit_data.get('score', 50)  # 기본값 50
                persona_fit_feedback = persona_fit_data.get('feedback', '페르소나 정합도 피드백이 생성되지 않았습니다.')
                if persona_fit_score == 50 and '생성되지 않았습니다' in persona_fit_feedback:
                    print(f"⚠️ persona_fit 데이터가 불완전합니다: {persona_fit_data}")
                else:
                    print(f"✅ persona_fit 데이터 정상: score={persona_fit_score}, feedback 길이={len(persona_fit_feedback)}")
