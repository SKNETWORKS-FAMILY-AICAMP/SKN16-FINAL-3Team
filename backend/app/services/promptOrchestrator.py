...
9. 대화가 7~10턴 이상 진행되면 자연스럽게 마무리하세요.
...
- 🚨 **급함 표현은 적절한 때만 사용하세요! (매우 중요)**
  * **첫 대화**: "빨리 처리할 수 있을까요?" ✅ (처음 1회 표현)
  * **일반 대화**: "네, 체크카드로 해주세요." ✅ (급함 표현 없이 자연스럽게)
  * **처리 중**: "네, 알겠습니다." ✅ (직원이 처리 중이면 그냥 기다림)
  * **실제 지연 발생**: "생각보다 오래 걸리네요" ✅ (진짜 오래 걸릴 때만)
  
- 🚨 **절대 금지: 모든 턴마다 "빨리" 반복하지 마세요!**
...
""".strip())
    ...
    if history:
        ...
        # 직원의 마무리 신호 감지 (실제 은행원이 사용하는 정중한 표현만)
        employee_closing_signals = sum(1 for item in history[-3:] if item.get('role') == 'employee' and any(
            phrase in item.get('text', '') for phrase in [
                "더 궁금하신 점",      # "더 궁금하신 점 있으세요?"
                "추가로 필요하신",     # "추가로 필요하신 게 있으세요?"
                "또 다른 문의",        # "또 다른 문의사항 있으신가요?"
                "다른 도움"            # "다른 도움 필요하신 부분 있으세요?"
            ]
        ))
        
        # 종료 조건: (10턴 이상 AND 주요 작업 완료) OR (직원이 2회 이상 마무리 신호)
        should_end_conversation = (turn_count >= 10 and major_tasks_completed) or employee_closing_signals >= 2
        
        if should_end_conversation:
            user_parts.append("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
            user_parts.append("🚨🚨🚨 [대화 종료 시점 - 반드시 마무리하세요!]\n")
            user_parts.append(f"현재 대화 턴 수: {turn_count}턴\n")
            user_parts.append(f"완료된 작업: {len(completed_tasks)}개\n")
            user_parts.append(f"직원 마무리 신호: {employee_closing_signals}회\n\n")
            user_parts.append("⚠️ 더 이상 질문하지 말고 대화를 마무리하세요!\n")
            ...
            user_parts.append("⚠️ end_signal을 true로 설정하세요!\n")
            user_parts.append("⚠️ 추가 질문을 하지 마세요!\n")
            user_parts.append("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
...
6. 🚨 마무리 신호 인식 및 응답 (매우 중요!):
   - 직원이 "더 궁금하신 점 있으세요?", "추가로 필요하신 게 있으세요?" 같은 질문을 하면:
     * 이건 **마무리 신호**입니다! 직원이 대화를 정리하려는 겁니다!
     * 정말 추가 질문이 있으면: 1개만 간단히 질문 후 바로 마무리
     * 없으면: "아니요, 잘 알겠습니다. 감사합니다!" 같은 마무리 응답
   - 🚨🚨🚨 **절대 금지: 이미 완료된 작업을 다시 요청하지 마세요!**
     * 직원: "카드 발급 완료했습니다. 더 궁금하신 점?" 
       → ❌❌❌ "카드 발급해주세요" (절대 금지!)
       → ✅✅✅ "아니요, 감사합니다!" 또는 새로운 질문 1개만
   - 직원이 2번 이상 마무리 질문을 하면 → 무조건 마무리하세요!
     * "네, 충분히 안내받았어요. 감사합니다!"
     * end_signal = true 설정
