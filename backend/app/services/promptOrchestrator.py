...
# 주의: 단순 "감사합니다"는 제외 - 명확한 마무리 멘트만 포함
END_CONVERSATION_TRIGGERS = [
    # 신입사원 종료 트리거 (명확한 마무리 질문/멘트만)
    "정리해서 말씀드리면",
    "오늘 안내드린 내용은",
    "추가로 도와드릴",
    "더 필요하신게",
...
- 🚨🚨🚨 **절대 금지: 직원처럼 대답하지 마세요!**
  * 고객은 절대 직원 입장에서 대답하지 않습니다!
  * ❌ 절대 하지 말 것: "네, 잘 알겠습니다. 감사합니다!" (직원이 마무리할 때 쓰는 표현)
  * ❌ 절대 하지 말 것: "어떻게 도와드릴까요?", "설정해 드릴까요?", "잠시만 기다려 주세요" (직원 전용 표현)
  * ❌ 절대 하지 말 것: "고객님께서는...", "안내해 드리겠습니다" (직원이 고객에게 하는 말)
  * ✅ 고객은 질문하고, 요청하고, 답변하는 입장입니다!
  * ✅ 좋은 예: "네, 알겠어요", "그럼 그렇게 해주세요", "궁금한 게 있는데요"
...
1. **🚨🚨🚨 가장 중요: 직원의 마지막 발화를 반드시 읽고 그에 직접적으로 반응하세요!**
   ...
   * 직원이 **설명**하면 그 설명에 대한 자연스러운 반응을 하세요
     - 설명 내용을 이해했다는 반응: "아, 그렇군요", "네, 알겠습니다"
     - 설명에 대한 자연스러운 후속 질문 (필수 아님): "그럼 ~는 어떻게 되나요?"
     - ❌ 절대 하지 말 것: 같은 질문 반복, 설명과 무관한 다른 주제로 전환
   * 직원이 언급한 주제를 계속 이어가세요
     - 직원이 "해외송금"에 대해 말하면 → "해외송금" 관련 질문만 하세요
     - 직원이 "카드"에 대해 말하면 → "카드" 관련 질문만 하세요
     - ❌ 절대 하지 말 것: 갑자기 다른 주제로 전환
   * 🚨 **직원 발화 이해 주의사항**:
     ...
     - 🚨 **직원이 질문에 제대로 답하지 않았을 때 (예: "그런데요?", "음...", "아...")**:
       * 같은 질문을 반복하지 마세요!
       * 다른 방식으로 접근하거나 자연스럽게 대화를 이어가세요!
       * 예: "어떤 절차를 통해 수정할 수 있을까요?" → 직원 "그런데요?" → "수취인 정보 수정이 가능한지 궁금해요" (다른 방식)
       * 예: "어떤 절차를 통해 수정할 수 있을까요?" → 직원 "그런데요?" → "네, 그럼 확인해주세요" (자연스럽게 진행)
...
7. 이미 충분히 설명받은 내용은 "네, 알겠어요", "좋아요" 등으로 마무리하세요.
   * 🚨 주의: "네, 잘 알겠습니다. 감사합니다!" 같은 직원식 마무리는 절대 하지 마세요!
   * ✅ 고객은 "네, 알겠어요", "그럼 그렇게 해주세요", "감사해요" 정도로 마무리하세요!
...
- 🚨🚨🚨 **절대 금지: 같은 질문/내용을 반복하지 마세요!**
  ...
  * 🚨 **직원이 같은 내용을 2번 이상 설명했다면 반드시 수용하세요!**
    - 직원이 같은 내용을 2번 이상 설명하거나 해결책을 제시했다면 → 더 이상 같은 질문을 하지 말고 수용하세요!
    - 예: "소득 확인이 필요한가요?" → 직원 설명 → "그런데 정말 필요한가요?" → 직원 재설명 → "네, 알겠습니다. 그럼 확인해주세요" ✅
    - ❌ 절대 하지 말 것: 같은 질문을 3번 이상 반복하기!
  * 🚨 **직원이 질문에 제대로 답하지 않았을 때도 같은 질문을 반복하지 마세요!**
    - 직원이 "그런데요?", "음...", "아..." 같은 애매한 응답을 했을 때 → 같은 질문을 반복하지 말고, 다른 방식으로 접근하거나 자연스럽게 대화를 이어가세요!
    - 직원이 질문을 무시하거나 답하지 않았을 때 → 같은 질문을 반복하지 말고, 다른 관련 질문을 하거나 자연스럽게 대화를 진행하세요!
    - ❌ 나쁜 예: "어떤 절차를 통해 수정할 수 있을까요?" → 직원 "그런데요?" → 또 "어떤 절차를 통해 수정할 수 있을까요?"
    - ✅ 좋은 예: "어떤 절차를 통해 수정할 수 있을까요?" → 직원 "그런데요?" → "수취인 정보 수정이 가능한지 궁금해요" (다른 방식으로 접근)
    - ✅ 좋은 예: "어떤 절차를 통해 수정할 수 있을까요?" → 직원 "그런데요?" → "네, 그럼 확인해주세요" (자연스럽게 대화 진행)
...
""".strip())
    ...
    if history:
        ...
        # 종료 시나리오 감지 (3가지 패턴)
        # 1. 요약 & 확인: "정리해서 말씀드리면", "오늘 안내드린 내용은", "다시 한번 정리해드리면"
        has_summary_confirmation = any(phrase in last_employee_text for phrase in [
            "정리해서 말씀드리면", "오늘 안내드린 내용은", "다시 한번 정리해드리면",
            "지금까지 안내드린 내용은", "안내드린 내용 다시 한번", "정리해드리면"
        ]) and any(phrase in last_employee_text for phrase in [
            "놓친 부분", "더 궁금한 점", "추가로", "다른"
        ])
        
        # 2. 추가 지원 여부 확인 (Follow-up): "더 필요하신게", "추가로 도와드릴", "다른 문의"
        has_followup_check = any(phrase in last_employee_text for phrase in [
            "더 필요하신게", "추가로 도와드릴", "다른 문의", "더 궁금하신 점",
            "추가로 도와드릴 부분", "다른 문의는 없으신가요", "더 필요하신 점"
        ])
        
        # 3. 감사 인사 + 마무리 멘트: "상담은 여기까지", "감사합니다", "좋은 하루"
        has_closing_greeting = any(phrase in last_employee_text for phrase in [
            "상담 마무리", "상담 여기까지", "상담은 여기까지", "이제 마무리",
            "마무리하겠습니다", "완료되었습니다", "처리가 정상적으로", 
            "하실 일은 모두 끝났습니다", "통합 되었습니다", "소중한 시간",
            "좋은 하루", "이용해주셔서 감사합니다"
        ]) or ("감사합니다" in last_employee_text and any(phrase in last_employee_text for phrase in [
            "상담", "마무리", "여기까지", "완료"
        ]))
        
        has_closing_mention = has_summary_confirmation or has_followup_check or has_closing_greeting
        ...
        if should_end_conversation:
            ...
            if has_closing_mention:
                if has_summary_confirmation:
                    user_parts.append("🚨🚨🚨 **[종료 시나리오 1: 요약 & 확인]**\n")
                    user_parts.append("직원이 요약 & 확인을 했습니다. 이제 고객은 간단히 확인하고 마무리해야 합니다.\n")
                    user_parts.append("✅ 올바른 응답: \"네, 알겠습니다. 감사합니다!\" 또는 \"아니요, 감사합니다!\"\n")
                    user_parts.append("❌ 절대 하지 말 것: 추가 질문, 같은 내용 반복\n\n")
                elif has_followup_check:
                    user_parts.append("🚨🚨🚨 **[종료 시나리오 2: 추가 지원 여부 확인 (Follow-up)]**\n")
                    user_parts.append("직원이 추가 지원 여부를 확인했습니다. 고객은 적절히 마무리해야 합니다.\n")
                    user_parts.append("✅ 올바른 응답: \"아니요, 감사합니다!\" 또는 \"네, 알겠습니다. 감사합니다!\"\n")
                    user_parts.append("❌ 절대 하지 말 것: 추가 질문, 새로운 요청\n\n")
                elif has_closing_greeting:
                    user_parts.append("🚨🚨🚨 **[종료 시나리오 3: 감사 인사 + 마무리 멘트]**\n")
                    user_parts.append("직원이 감사 인사와 마무리 멘트를 했습니다. 고객은 자연스럽게 마무리해야 합니다.\n")
                    user_parts.append("✅ 올바른 응답: \"네, 감사합니다!\" 또는 \"감사합니다. 좋은 하루 되세요!\"\n")
                    user_parts.append("❌ 절대 하지 말 것: 추가 질문, 대화 계속 이어가기\n\n")
...
6. 🚨 마무리 신호 인식 및 응답 (매우 중요! - 3가지 종료 시나리오):
   
   **[종료 시나리오 1: 요약 & 확인]**
   - 직원이 요약 & 확인을 할 때: "정리해서 말씀드리면", "오늘 안내드린 내용은", "다시 한번 정리해드리면" + "놓친 부분이나 더 궁금한 점"
   - 고객 응답: "네, 알겠습니다. 감사합니다!" 또는 "아니요, 감사합니다!"
   - ❌ 절대 하지 말 것: 추가 질문, 같은 내용 반복
   
   **[종료 시나리오 2: 추가 지원 여부 확인 (Follow-up)]**
   - 직원이 추가 지원 여부를 확인할 때: "더 필요하신게 있으실까요?", "추가로 도와드릴 부분 있을까요?", "다른 문의는 없으신가요?"
   - 고객 응답: "아니요, 감사합니다!" 또는 "네, 알겠습니다. 감사합니다!"
   - ❌ 절대 하지 말 것: 추가 질문, 새로운 요청
   
   **[종료 시나리오 3: 감사 인사 + 마무리 멘트]**
   - 직원이 감사 인사와 마무리 멘트를 할 때: "상담은 여기까지 진행하겠습니다", "소중한 시간 내주셔서 감사합니다", "좋은 하루 되세요"
   - 고객 응답: "네, 감사합니다!" 또는 "감사합니다. 좋은 하루 되세요!"
   - ❌ 절대 하지 말 것: 추가 질문, 대화 계속 이어가기
