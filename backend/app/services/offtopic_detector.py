...
def is_on_topic(utterance: str) -> bool:
    ...
    # 🚨 중요: 은행 키워드가 하나라도 포함되어 있으면 온토픽 (최우선!)
    for keyword in BANK_KEYWORDS:
        if keyword in utterance_lower:
            return True  # 은행 업무 관련이면 무조건 온토픽
    ...
    # 🚨 수정: 명백한 잡담/이탈 키워드가 있는 경우에만 이탈로 판단
    # "어려워", "쉬워" 같은 단어는 은행 업무 설명 중에도 자주 나오므로 제외
    offtopic_keywords = [
        "맛있", "먹고", "음식", "밥", "배고", "배고파", "배고픔", "맛", "식당", "식사",
        "영화", "드라마", "여행", "주말", "휴가", "운동", "게임", "노래", "책",
        ...
        "재밌", "재미", "즐거", "즐겁", "즐거워",
        "피곤", "졸려", "잠", "자고", "잠자"
        # "어려워", "어렵", "쉬워", "쉬운" 제거 (은행 업무 설명 중 자주 사용)
        # "힘들" 제거 (은행 업무 중 "힘드시면" 같은 표현 사용)
    ]
